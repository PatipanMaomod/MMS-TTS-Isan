...
TRAIN_CSV_FIELDS = [
    "global_step", "epoch", "lr",
    "train_summed_losses",
    "train_loss_duration", "train_loss_mel", "train_loss_kl",
    "train_loss_fmaps", "train_loss_gen",
    "train_loss_disc", "train_loss_real_disc", "train_loss_fake_disc",
]

EVAL_CSV_FIELDS = [
    "global_step", "epoch",
    "val_loss_mel", "val_loss_kl", "val_loss_mel_kl",
]


def init_csv_logger(output_dir: str):
    """
    สร้างไฟล์ CSV สำหรับ train และ eval losses
    คืนค่า (train_csv_path, eval_csv_path)
    """
    os.makedirs(output_dir, exist_ok=True)
    train_csv_path = os.path.join(output_dir, "train_losses.csv")
    eval_csv_path  = os.path.join(output_dir, "eval_losses.csv")

    # เขียน header ถ้าไฟล์ยังไม่มี (รองรับ resume จาก checkpoint)
...
def append_train_loss_to_csv(train_csv_path: str, row: dict):
    """เพิ่ม 1 แถวของ train loss ลงไฟล์ CSV"""
    with open(train_csv_path, "a", newline="") as f:
        writer = csv.DictWriter(f, fieldnames=TRAIN_CSV_FIELDS)
        writer.writerow(row)


def append_eval_loss_to_csv(eval_csv_path: str, row: dict):
    """เพิ่ม 1 แถวของ eval loss ลงไฟล์ CSV"""
...
def main():
    ...
    # ── เริ่มต้น CSV logger (เฉพาะ main process) ─────────────────────────────
    train_csv_path, eval_csv_path = None, None
    if accelerator.is_main_process:
        train_csv_path, eval_csv_path = init_csv_logger(training_args.output_dir)
        logger.info(f"CSV loss logs will be saved to: {training_args.output_dir}/train_losses.csv  |  eval_losses.csv")
    ...
    for epoch in range(first_epoch, training_args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            if accelerator.sync_gradients:
                ...
                # ── บันทึก train loss ลง CSV (เฉพาะ main process) ──────────
                if accelerator.is_main_process:
                    append_train_loss_to_csv(
                        train_csv_path,
                        {
            ...
            if do_eval:
                ...
                if accelerator.is_main_process:
                    ...
                    # ── บันทึก eval loss ลง CSV ────────────────────────────
                    append_eval_loss_to_csv(
                        eval_csv_path,
                        {
                            "global_step":    global_step,
                            "epoch":          epoch,
                            "val_loss_mel":   val_losses.get("val_loss_mel", ""),
                            "val_loss_kl":    val_losses.get("val_loss_kl", ""),
                            "val_loss_mel_kl": val_losses.get("val_loss_mel_kl", ""),
                        },
                    )
